...
def create_app():
    ...
    def create_client():
        ...
        client = Client(name=name, surname=surname, credit_card=credit_card, car_number=car_number)
    ...
    def create_parking():
        ...
        if not address or count_places is None or count_available_places is None:
            return jsonify({'error': 'Поля address, count_places и count_available_places обязательны'}), 400

        if count_available_places > count_places:
            return jsonify({'error': 'count_available_places не может быть больше count_places'}), 400
    ...
    def enter_parking():
        ...
        active_entry = ClientParking.query.filter_by(client_id=client_id, parking_id=parking_id, time_out=None).first()
        if active_entry:
        ...
        return jsonify({'message': 'Заезд зафиксирован', 'client_parking_id': cp.id}), 201
    ...
    def exit_parking():
        ...
        if not client.credit_card:
            return jsonify({'error': 'У клиента не привязана кредитная карта, оплата невозможна'}), 400
